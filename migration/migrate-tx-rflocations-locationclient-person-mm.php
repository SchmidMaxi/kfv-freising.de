...
echo "Migrating table tx_rflocations_locationclient_person_mm..." . PHP_EOL;

$queryBuilder = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\TYPO3\CMS\Core\Database\ConnectionPool::class)
    ->getQueryBuilderForTable('tx_rflocations_locationclient_person_mm');
$queryBuilder
    ->select('*')
    ->from('tx_rflocations_locationclient_person_mm')
...
$queryBuilder = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\TYPO3\CMS\Core\Database\ConnectionPool::class)
    ->getQueryBuilderForTable('tx_rflocations_locationclient_person_mm');
$queryBuilder
    ->delete('tx_rflocations_locationclient_person_mm')
    ->executeStatement();
...
foreach ($result as $row) {
    if($previousUidLocal !== $row['uid_local'] || $previousUidForeign !== $row['uid_foreign']) {
        ...
        $queryBuilder = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\TYPO3\CMS\Core\Database\ConnectionPool::class)
            ->getQueryBuilderForTable('tx_rflocations_locationclient_person_mm');
        $queryBuilder
            ->insert('tx_rflocations_locationclient_person_mm')
            ->values([
