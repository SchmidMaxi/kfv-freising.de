...
namespace Trion\ScssCompiler\Parser;
...
class ScssParser extends AbstractParser
{
    ...
    public function __construct()
    {
        if (!class_exists('ScssPhp\ScssPhp\Version')) {
            require_once ExtensionManagementUtility::extPath('scss_compiler') . '/Contrib/scssphp/scss.inc.php';
        }
    }
    ...
    protected function parseFile(string $file, array $settings): array
    {
        ...
        $packageName = $this->getPackageName($settings['file']['relative']);
        $absoluteBootstrapPackageThemePath =
            ExtensionManagementUtility::extPath($packageName) . 'Resources/Public/Scss/Theme';
    ...
    protected function getPackageName(string $file) {
        return explode('/', str_replace('EXT:', '', $file))[0];
    }
}
